...
{
	uvc_trace(UVC_TRACE_FRAME, "uvc_query_name(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
	switch (query) {
	case UVC_SET_CUR:
		return "SET_CUR";
	case UVC_GET_CUR:
...
{
	uvc_trace(UVC_TRACE_FRAME, "uvc_get_video_ctrl(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
...
{
	uvc_trace(UVC_TRACE_FRAME, "uvc_set_video_ctrl(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
	u16 size = uvc_video_ctrl_size(stream);
	u8 *data;
	int ret;
	...
	struct uvc_streaming_control *probe)
{
	...
	uvc_trace(UVC_TRACE_FRAME, "uvc_probe_video(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
...
static void uvc_video_stats_decode(struct uvc_streaming *stream,
		const u8 *data, int len)
{
	uvc_trace(UVC_TRACE_FRAME, "uvc_video_stats_decode(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
	unsigned int header_size;
	bool has_pts = false;
	bool has_scr = false;
...
{
	uvc_trace(UVC_TRACE_FRAME, "uvc_video_decode_start(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
...
int uvc_video_init(struct uvc_streaming *stream)
{
	...
	uvc_trace(UVC_TRACE_FRAME, "uvc_video_init(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
...
{
	int ret;
	uvc_trace(UVC_TRACE_FRAME, "uvc_video_start_streaming(): kernel analytics variable-value : %s = %s \n",virgo_kernel_analytics_conf[0].key,virgo_kernel_analytics_conf[0].value);
